...
public class QuestionHis extends JFrame implements ActionListener {
    ...
    QuestionHis(){
        ...
        setSize(1200,650);
        setTitle("Question History - "+LoginPage.FULL_NAME);
...
//         table impliment
        String[] col = {"Sl No.", "Asked Date","Question","Admin Comment"};
        ...
        hhead = new JLabel("ASKED QUESTION HISTORY");
        hhead.setForeground(Color.WHITE);
        hhead.setFont(new Font("Times New Roman",Font.PLAIN,28));
        hhead.setBounds(20,0,400,50);
        hpanel.add(hhead);

        //table query JDBC
        String query1 = "Select * from questionask where username ='"+username2+"'";
        try {
            ...
            while (rs1.next()){
                ...
                String queAns = rs1.getString(5);
                model.addRow(new Object[]{slno,date,que,queAns});
                i++;
                count++;
            }
            if (i < 1) {
